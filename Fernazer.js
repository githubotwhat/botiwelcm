...
frnzer.on('chat-update', async (frnazer) => {
		try {
			...
			const ownerNumber = ["6282138919347@s.whatsapp.net"] // Nomor Owner Bre
			const ownerContact = ["6282138919347;","6282138919347"] // Nomor Owner Bre Yg Bentuk Kode Jangan Di Ganti
        ...
        let id_vote = sender ? sender : '6282138919347@s.whatsapp.net'
...
frnzer.sendMessage(from,{contentText:`Hai,${userfernazer}${ucapanWaktu}`,footerText:'LIST SEWA BOT BY HELGA GANZ',buttons:[{buttonId:`${prefix}seminggu`,buttonText:{displayText:'1 MINGGU'},type:1},{buttonId:`${prefix}sebulan`,buttonText:{displayText:'1 BULAN'},type:1}],headerType:'LOCATION',locationMessage:{degreesLatitude:'',degreesLongitude:'',jpegThumbnail:tid,contextInfo:{mentionedJid:[sender]}}},'buttonsMessage') 
break

case 'seminggu':
if (isBanned) return reply (mess.only.banned)
frnzer.sendButton(from,'SEWA BOT 1 MINGGU 5K SILAHKAN PILIH PEMBAYARAN DU BAWAH INI','Creator: Fernazer','GOPAY️',`${prefix}gopay`,{quoted: faketroli})
break

case 'sebulan':
if (isBanned) return reply (mess.only.banned)
frnzer.sendButton(from,'SEWA BOT 1 BULAN 10k SILAHKAN PILIH PEMBAYARAN DI BAWAH INI','Creator: Fernazer','GOPAY️',`${prefix}gopay`,{quoted: faketroli})
break

case 'gopay':
if (isBanned) return reply (mess.only.banned)
frnzer.sendButton(from,`NODANA ${danaa}`,'Creator: Hanbu FF','DONE',`${ucapanWaktu}`,{quoted: faketroli})
break
/******************************* FERNAZER *******************************/
/******************************* RETURN RANDOM *******************************/
...
addCmd(kodenya, q)
reply("Done Jangan Lupa Subrek Hanbu FF\n\nLink Yt: https://youtube.com/channel/UCIvY2o-KctmlKWvXzqrXuWw !")
} else {
reply('tag stickenya')
}
break
...
case 'stickergif':
case 'stikergif':
/* ERROR BILANG ATAU HUBUNGI: 081398860278 */
...
case 'doge':
reply(mess.wait)
fetch('https://raw.githubusercontent.com/HelgaIlham/rafli/main/data')
.then(res => res.text())
.then(body => {
let tod = body.split("\n");
...
case 'sticknime':
reply(mess.wait)
fetch('https://raw.githubusercontent.com/HelgaIlham/rafli/main/sticknime')
.then(res => res.text())
.then(body => {
let tod = body.split("\n");
let pjr = tod[Math.floor(Math.random() * tod.length)];
...
frnzer.sendButton(from,ucapanWaktu,'Creator: Hanbu FF','NEXT➡',`${prefix}doge`,{quoted: faketroli})
break	                
...
case 'randomnime':
reply(mess.wait)
fetch('https://raw.githubusercontent.com/HelgaIlham/rafli/main/randomnime')
.then(res => res.text())
.then(body => {
let tod = body.split("\n");
let pjr = tod[Math.floor(Math.random() * tod.length)];
...
frnzer.sendButton(from, teks,'Creator: Hanbu FF','KEMBALI KE MENU↩️',`${prefix}menu2`,{quoted: faketroli})
break
case 'doaharian':         							
frnzer.updatePresence(from, Presence.composing) 
reply(mess.wait)
